...
function supersededFixes(service) {
    ...
    // If possible, decied based on actually available features rather than UXP platform version.
...
class PolyfillService {
    constructor() {
        ...
        // - queueMicrotask
        //this.hasqueueMicrotask = typeof window.queueMicrotask === "function";
        this.hasqueueMicrotask = Services.vc.compare(Services.appinfo.platformVersion, "4.8.5") >= 0;
        // - Optional Chaining Operator (?.)
        var valid = true; try { new Function("this?.fourtytwo"); } catch (exc) { valid = false; }
        this.hasOptionalChainingOperator = valid;
        // - Nullish coalessing operator.
        var valid = true; try { new Function("var x = 42 ?? 0"); } catch (exc) { valid = false; }
        this.hasNullishCoalescingOperator = valid;
...
function _optionsDisplayed(xul) {
    ...
    btnDumpPlatform.addEventListener("click", () => {
        const info = {
            ...
            "Available browser features": toJSON(gService, "has"),
